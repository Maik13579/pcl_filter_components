...
#ifndef PCL_FILTER_COMPONENTS__FILTERS__SELECTION__REMOVE_NAN_FILTER_HPP_
#define PCL_FILTER_COMPONENTS__FILTERS__SELECTION__REMOVE_NAN_FILTER_HPP_
...
private:
  ...
  {
    const bool has_nan = std::isnan(p.x) || std::isnan(p.y) || std::isnan(p.z);
    return params_.invert ? has_nan : !has_nan;
  }
...
#endif  // PCL_FILTER_COMPONENTS__FILTERS__SELECTION__REMOVE_NAN_FILTER_HPP_
